"""YCLIENTS MCP tools: Comments — user comments on companies and staff"""
...
OPERATIONS: dict[str, tuple[str, str, bool]] = {
    "get_comments": ("GET", "/api/v1/comments/{company_id}/", True),
    "create_comments": ("POST", "/api/v1/comments/{company_id}/{staff_id}", True),
}


def register(mcp: FastMCP, client: YClientsClient) -> None:
    @mcp.tool()
    async def yclients_comments(
        operation: str, params: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        """Comments — user comments on companies and staff

        Available operations:
          - get_comments: [GET] Получить комментарии
          - create_comments: [POST] Оставить комментарии
