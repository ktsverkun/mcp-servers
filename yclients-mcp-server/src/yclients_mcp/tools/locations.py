"""YCLIENTS MCP tools: Locations — countries, cities, business type directories"""
...
OPERATIONS: dict[str, tuple[str, str, bool]] = {
    "get_references_business_groups_with_types": (
        "GET",
        "/api/v1/references/business_groups_with_types",
        True,
    ),
    "list_countries": ("GET", "/api/v1/countries", False),
    "list_cities_country_id_1_company_id_1": (
        "GET",
        "/api/v1/cities?country_id=1&company_id=1",
        False,
    ),
}
...
def register(mcp: FastMCP, client: YClientsClient) -> None:
    @mcp.tool()
    async def yclients_locations(
        operation: str, params: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        """Locations — countries, cities, business type directories

        Available operations:
          - get_references_business_groups_with_types: [GET] Получить бизнес-типы по группам
          - list_countries: [GET] Получить список стран
          - list_cities_country_id_1_company_id_1: [GET] Получить список городов
