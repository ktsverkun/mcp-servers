"""YCLIENTS MCP tools: Licenses — manage company licenses"""
...
OPERATIONS: dict[str, tuple[str, str, bool]] = {
    "get_license": ("GET", "/api/v1/license/{company_id}", True),
}


def register(mcp: FastMCP, client: YClientsClient) -> None:
    @mcp.tool()
    async def yclients_licenses(
        operation: str, params: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        """Licenses — manage company licenses

        Available operations:
          - get_license: [GET] Получение информация о лицензии компании
