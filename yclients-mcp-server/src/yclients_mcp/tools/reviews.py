"""YCLIENTS MCP tools: Reviews — manage reviews and tips"""
...
OPERATIONS: dict[str, tuple[str, str, bool]] = {
    "get_master_record_review": ("GET", "/api/v1/master_record_review/{recordToken}", False),
    "create_master_record_review": ("POST", "/api/v1/master_record_review/{recordToken}", True),
}


def register(mcp: FastMCP, client: YClientsClient) -> None:
    @mcp.tool()
    async def yclients_reviews(
        operation: str, params: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        """Reviews — manage reviews and tips

        Available operations:
          - get_master_record_review: [GET] Получение статуса формы отзыва
          - create_master_record_review: [POST] Отправка формы отзыва
