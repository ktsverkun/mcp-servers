"""YCLIENTS MCP tools: Salon Chains — manage salon chain networks"""
...
OPERATIONS: dict[str, tuple[str, str, bool]] = {
    "get_groups": ("GET", "/api/v1/groups", True),
}


def register(mcp: FastMCP, client: YClientsClient) -> None:
    @mcp.tool()
    async def yclients_salon_chains(
        operation: str, params: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        """Salon Chains — manage salon chain networks

        Available operations:
          - get_groups: [GET] Получение доступных пользователю сетей
