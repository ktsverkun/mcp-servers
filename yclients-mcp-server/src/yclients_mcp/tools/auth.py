"""YCLIENTS MCP tools: Authentication — authorize users and get tokens"""
...
OPERATIONS: dict[str, tuple[str, str, bool]] = {
    "auth_auth": ("POST", "/api/v1/auth", False),
}


def register(mcp: FastMCP, client: YClientsClient) -> None:
    @mcp.tool()
    async def yclients_auth(operation: str, params: dict[str, Any] | None = None) -> dict[str, Any]:
        """Authentication — authorize users and get tokens

        Available operations:
          - auth_auth: [POST] Авторизовать пользователя
