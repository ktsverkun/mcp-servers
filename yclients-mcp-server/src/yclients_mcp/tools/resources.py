"""YCLIENTS MCP tools: Resources — manage booking resources"""
...
OPERATIONS: dict[str, tuple[str, str, bool]] = {
    "get_resources": ("GET", "/api/v1/resources/{company_id}", True),
}


def register(mcp: FastMCP, client: YClientsClient) -> None:
    @mcp.tool()
    async def yclients_resources(
        operation: str, params: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        """Resources — manage booking resources

        Available operations:
          - get_resources: [GET] Получение ресурсов в филиале
